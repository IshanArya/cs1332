...
public class TestMM {
    ...
    public void sets() {
        try {
            ...
            System.out.println("Please test the put() test first.");
            throw e;
        }
        ...
        try {
            ...
            System.out.println("Please test the remove() method first.");
            throw e;
        }
        keys = map.keySet();
    ...
    private String[] readWords() {
        ...
        try (BufferedReader r = Files.newBufferedReader(Paths.get(PATH), Charset.defaultCharset())) {
            r.lines().forEach(s -> {
                ...
                while (m.find()) list.addLast(m.group().replaceAll("[\\(\\)\\.]", ""));
            });
    ...
    private MapEntry<Integer, Integer> e(int a, int b) {
        return new MapEntry<>(a, b);
    }

    private MapEntry<Integer, Integer> d(int a, int b) {
        MapEntry<Integer, Integer> m = e(a, b);
        m.setRemoved(true);
        return m;
    }
}