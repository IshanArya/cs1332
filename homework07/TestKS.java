...
public class TestKS {
    ...
    private int verifyTree(AVLNode<Integer> tree) {
        ...
        try {
            assertEquals( Math.max(heightLeft, heightRight) + 1, tree.getHeight());
            assertEquals(heightLeft - heightRight, tree.getBalanceFactor());
            oldTree = tree;
        } catch(AssertionError e) {
            Stack<AVLNode<Integer>> stack = new Stack<>();
            stack.push(tree);
            while(!stack.isEmpty()) {
                AVLNode<Integer> current = stack.pop();
